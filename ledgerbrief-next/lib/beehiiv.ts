...
// Native fetch has no default timeout — an unresponsive network path (build
// sandbox or production) can hang a request indefinitely, which is exactly
// what blew past Next's static-generation budget. Every Beehiiv call gets a
// hard timeout so a slow/unreachable API fails fast into the existing
// null/[] fallback instead of hanging the caller (build-time or request-time).
